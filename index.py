...
def main(argv):
    ...
    if not os.path.isfile(secret_key_file):
        raise Exception('Missing required secret key JSON file. Please check file name in settings.py or create one '
    ...
    analytics_service = GetAnalyticsService('analytics', 'v3', analytics_scope, secret_key_file)
    ...
    # Authenticate and construct service.
    tag_manager_service = GetService('tagmanager', 'v1', tag_manager_scope, secret_key_file)
